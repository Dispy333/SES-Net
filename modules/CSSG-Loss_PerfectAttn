def build_golden_attention_from_bbox_by_class(targets, img_shape, feat_shape, patch_size, device, B):
    """
    Args:
        targets: dict 包含 'cls' (N,1), 'bboxes' (N,4), 'batch_idx' (N,)
        img_shape: 原图尺寸 (H, W)
        feat_shape: 特征图尺寸 (Hf, Wf)
        patch_size: patch 大小
        device: torch.device
